...
import Cardano.Binary (FromCBOR (..), ToCBOR (..), DecoderError(DecoderErrorCustom), encodeListLen)
...
import Cardano.Prelude (forceElemsToWHNF, cborError)
...
instance Era era => FromCBOR (ShelleyGenesis era) where
  fromCBOR = do
    decodeRecordNamed "ShelleyGenesis" (const 15) $ do
      ...
      when (sgActiveSlotsCoeff == minBound) $
        cborError $ DecoderErrorCustom "sgActiveSlotsCoeff" "Zero value is not acceptable"
