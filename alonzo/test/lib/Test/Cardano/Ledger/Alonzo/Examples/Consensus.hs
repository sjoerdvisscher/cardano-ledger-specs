...
ledgerExamplesAlonzo =
  SLE.ShelleyLedgerExamples
    { SLE.sleBlock = SLE.exampleShelleyLedgerBlock exampleTransactionInBloc,
...
exampleMultiAssetValue x =
  ...
  where
    policyId :: PolicyID c
    policyId = PolicyID $ SLE.mkScriptHash x
...
exampleTransactionInBloc :: ValidatedTx StandardAlonzo
exampleTransactionInBloc = ValidatedTx b w (IsValidating True) a
  where
    (Tx b w a) = exampleTx
...
exampleAlonzoNewEpochState =
  SLE.exampleNewEpochState
    ...
    emptyPParams
    (emptyPParams {_adaPerUTxOWord = Coin 1})
